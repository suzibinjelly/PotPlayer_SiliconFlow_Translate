/*
    Real-time subtitle translation for PotPlayer using SiliconFlow API
*/

// Plugin Information Functions
string GetTitle() {
    return "{$CP0=SiliconFlow Translate$}";
}
...
string GetDesc() {
    return "{$CP0=Real-time subtitle translation using SiliconFlow$}";
}

string GetLoginTitle() {
    return "{$CP0=OpenAI Model and API Key Configuration$}";
}

string GetLoginDesc() {
    return "{$CP0=Please enter the model name and provide the API Key (e.g., Qwen/Qwen2.5-7B-Instruct).$}";
}

string GetUserText() {
    return "{$CP0=Model Name (Current: " + selected_model + ")$}";
}
...
// Global Variables
string api_key = "";
string selected_model = "Qwen/Qwen2.5-7B-Instruct"; // Default model
string UserAgent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64)";
string apiUrl = "https://api.siliconflow.cn/v1/chat/completions"; // Added apiUrl definition
...
string ServerLogin(string User, string Pass) {
    ...
    if (selected_model.empty()) {
        ...
        selected_model = "Qwen/Qwen2.5-7B-Instruct"; // Default to Qwen/Qwen2.5-7B-Instruct
    }
...
string Translate(string Text, string &in SrcLang, string &in DstLang) {
    ...
    selected_model = HostLoadString("selected_model", "Qwen/Qwen2.5-7B-Instruct");
...
void OnInitialize() {
    HostPrintUTF8("{$CP0=SiliconFlow translation plugin loaded.$}\n");
...
// Plugin Finalization
void OnFinalize() {
    HostPrintUTF8("{$CP0=SiliconFlow translation plugin unloaded.$}\n");
}
